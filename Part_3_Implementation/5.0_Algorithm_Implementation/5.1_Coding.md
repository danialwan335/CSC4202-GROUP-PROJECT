...
The program models the post-landslide road network as a weighted undirected graph and runs Dijkstra's Algorithm from Hospital Kuala Kubu Bharu (Node 0) to Kampung Sungai Lui (Node 7).

It demonstrates **three test scenarios** to validate correctness and edge cases (as required by Section 4.10 — Program Testing):

| Scenario | Description | Expected Output |
|---|---|---|
| **A** | Base post-landslide network (9 surviving edges, main highway blocked) | Optimal route: 0→1→3→5→7 in **55 minutes** |
| **B** | Additional road closure — Sungai Chilling road (1→3) also blocked | Re-routes via plantation: 0→1→2→4→6→7 in **62 minutes** |
| **C** | Village completely isolated — no surviving path exists | Reports no path, recommends helicopter evacuation |
...
public class LandslideEvacuation {
    ...
    //  dist[] — shortest travel time from source to every node
    ...
    static void printResult(Graph g, int src, int dest, String label) {
        ...
        System.out.printf("║  Destination : Node %-2d — %-37s║%n", dest, NODE_NAMES[dest]);
        System.out.println("╠══════════════════════════════════════════════════════════════════╣");
        ...
        for (int i = 0; i < path.size(); i++) {
            ...
            if (i < path.size() - 1) routeSb.append(" → ");
        }
        ...
        for (int i = 0; i < path.size() - 1; i++) {
            ...
            int w = 0;
            for (int[] e : g.adj[u]) if (e[0] == v) { w = e[1]; break; }
            total += w;
...
1. **Adjacency List over Adjacency Matrix** — The post-landslide network is sparse (9 edges, 8 nodes). An adjacency list uses O(V + E) space vs O(V²) for a matrix, making it more efficient for sparse graphs.

2. **Min-Priority Queue (Binary Heap)** — Java's `PriorityQueue` gives O(log n) extraction and insertion, resulting in overall O((V + E) log V) time — significantly better than the O(V²) naive array approach.

3. **Lazy Deletion for Stale Entries** — Instead of implementing a complex `decreaseKey()` operation, stale priority queue entries (old copies of a node with a larger distance) are skipped using the `visited[]` array. This is a standard, correct, and simpler approach.

4. **Three Test Scenarios** — Scenario A is the main demonstration. Scenarios B and C satisfy the program testing requirement (Section 4.10) by covering edge cases: an additional road closure and a completely isolated village.
