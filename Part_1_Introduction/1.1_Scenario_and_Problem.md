...
To find the **minimum-time route** for the ambulance from Hospital Kuala Kubu Bharu to Kampung Sungai Lui through the surviving secondary road network, so the patient can receive cardiac intervention within the critical 90-minute window.

## Expected Output

- The **optimal sequence of road junctions** forming the fastest route (e.g., Node 0 → 1 → 3 → 5 → 7).
- The **total minimum travel time** in minutes.
- A **segment-by-segment breakdown** of travel time for each road used.
- If no route exists, a notification that the village is isolated and helicopter evacuation should be requested.

These outputs support emergency dispatch decision-making in real time.
